...
_global_pop_view = None
_global_matrix_view = None
_global_N = 0
_global_num_pairs = 0
_global_K = 0
_global_adjacency = {}
...
# --- Funciones del Worker (¡MUÉVELAS AQUÍ!) ---

def _init_parallel_worker(
    N, num_pairs, K, adjacency,
    pop_shm_name, pop_shape, pop_dtype,
    matrix_shm_name, matrix_shape, matrix_dtype
):
    """
    Inicializador del worker.
    """
    global _global_pop_view, _global_matrix_view
    global _global_N, _global_num_pairs, _global_K, _global_adjacency
    ...
    # 2. Almacenar datos pequeños
    _global_N = N
    _global_num_pairs = num_pairs
    _global_K = K
    _global_adjacency = adjacency

    # 3. Conectarse a SHM
    matrix_shm = shared_memory.SharedMemory(name=matrix_shm_name)
    _global_matrix_view = np.ndarray(matrix_shape, dtype=matrix_dtype, buffer=matrix_shm.buf)
    
    pop_shm = shared_memory.SharedMemory(name=pop_shm_name)
    _global_pop_view = np.ndarray(pop_shape, dtype=pop_dtype, buffer=pop_shm.buf)



def _run_parallel_worker(idx: int) -> float:
    """
    Función del worker.
    """
    chromosome = _global_pop_view[idx] # type: ignore
    return chromosome_fitness(
        chromosome,
        _global_matrix_view, # type: ignore
        _global_N,
        _global_num_pairs,
        _global_K,
        _global_adjacency
