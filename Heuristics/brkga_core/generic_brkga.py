...
class GenericBRKGA(ABC):
    ...
    def evaluate_population(self, population: np.ndarray) -> np.ndarray:
        """
        Evaluate fitness for all chromosomes in the given population.

        Args:
            population (np.ndarray): Population matrix of shape (p, n)

        Returns:
            np.ndarray: Fitness values of shape (p,)
        """
        return np.apply_along_axis(self.chromosome_fitness, axis=1, arr=population) # type: ignore
    ...
    def update_population(self, old_fitness_values: np.ndarray,
        ...
        fitness_values[self.p_e:] = self.evaluate_population(population[self.p_e:])
